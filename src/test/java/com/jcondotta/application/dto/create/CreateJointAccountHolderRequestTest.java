...
class CreateJointAccountHolderRequestTest {
    ...
    void shouldNotDetectConstraintViolation_whenRequestIsValid() {
        var request = new CreateJointAccountHolderRequest(
            new AccountHolderDetailsRequest(ACCOUNT_HOLDER_NAME_JEFFERSON, DATE_OF_BIRTH_JEFFERSON, PASSPORT_NUMBER_JEFFERSON)
        );
    ...
    void shouldDetectConstraintViolation_whenAccountHolderNameIsBlank(String blankAccountHolderName) {
        var request = new CreateJointAccountHolderRequest(
            new AccountHolderDetailsRequest(blankAccountHolderName, DATE_OF_BIRTH_JEFFERSON, PASSPORT_NUMBER_JEFFERSON)
        );
    ...
    void shouldDetectConstraintViolation_whenAccountHolderNameIsLongerThan255Characters() {
        ...
        var request = new CreateJointAccountHolderRequest(
            new AccountHolderDetailsRequest(veryLongAccountHolderName, DATE_OF_BIRTH_JEFFERSON, PASSPORT_NUMBER_JEFFERSON)
        );
        var exception = assertThrowsExactly(ConstraintViolationException.class, () ->
                request.validateWith(VALIDATOR)
        );
    ...
    void shouldDetectConstraintViolation_whenDateOfBirthIsInFuture() {
        ...
        var request = new CreateJointAccountHolderRequest(
            new AccountHolderDetailsRequest(ACCOUNT_HOLDER_NAME_JEFFERSON, futureDate, PASSPORT_NUMBER_JEFFERSON)
        );
        var exception = assertThrowsExactly(ConstraintViolationException.class, () ->
                request.validateWith(VALIDATOR)
        );
    ...
    void shouldDetectConstraintViolation_whenDateOfBirthIsToday() {
        ...
        var request = new CreateJointAccountHolderRequest(
            new AccountHolderDetailsRequest(ACCOUNT_HOLDER_NAME_JEFFERSON, today, PASSPORT_NUMBER_JEFFERSON)
        );
        var exception = assertThrowsExactly(ConstraintViolationException.class, () ->
                request.validateWith(VALIDATOR)
        );
    ...
    void shouldDetectConstraintViolation_whenPassportNumberIsNull() {
        var request = new CreateJointAccountHolderRequest(
            new AccountHolderDetailsRequest(ACCOUNT_HOLDER_NAME_JEFFERSON, DATE_OF_BIRTH_JEFFERSON, null)
        );
        var exception = assertThrowsExactly(ConstraintViolationException.class, () ->
                request.validateWith(VALIDATOR)
        );
    ...
    void shouldDetectConstraintViolation_whenPassportNumberIsNot8CharactersLong(String invalidLengthPassportNumber) {
        var request = new CreateJointAccountHolderRequest(
            new AccountHolderDetailsRequest(ACCOUNT_HOLDER_NAME_JEFFERSON, DATE_OF_BIRTH_JEFFERSON, invalidLengthPassportNumber)
        );
        var exception = assertThrowsExactly(ConstraintViolationException.class, () ->
                request.validateWith(VALIDATOR)
        );
    ...
    void shouldDetectMultipleConstraintViolation_whenAllFieldsAreNull() {
        var request = new CreateJointAccountHolderRequest(
            new AccountHolderDetailsRequest(null, null, null)
        );
